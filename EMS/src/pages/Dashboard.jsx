...
const Dashboard = ({ user = { role: 'user' } }) => {
  ...
  <div className="relative min-h-screen w-full overflow-hidden">
    ...
    <div className="relative z-20 w-full max-w-7xl mx-auto px-4 sm:px-6 py-8 flex flex-col gap-8">
      <div className="bg-white/90 backdrop-blur-md rounded-2xl shadow-2xl p-6 md:p-10 flex flex-col items-center border border-blue-100">
        <h1 className="text-3xl md:text-4xl font-extrabold text-blue-800 text-center mb-2 tracking-tight drop-shadow">
          Dashboard
        </h1>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        <div className="w-full bg-gradient-to-br from-blue-100 to-blue-50/80 rounded-xl shadow-lg p-6 flex flex-col items-center border border-blue-200">
        ...
        <div className="w-full bg-gradient-to-br from-green-100 to-green-50/80 rounded-xl shadow-lg p-6 border border-green-200">
            ...
            <ul className="list-disc pl-5 space-y-2">
              {stats.recentHires.map((hire, index) => (
                <li key={index} className="text-gray-800">
                  <span className="font-semibold text-green-800">{hire.name}</span> - {hire.position}{' '}
                  <span className="text-gray-500">({hire.department})</span>
                  <br />
        ...
        <div className="w-full bg-gradient-to-br from-yellow-100 to-yellow-50/80 rounded-xl shadow-lg p-6 border border-yellow-200">
            ...
            {stats.activityLogs.map((log, index) => (
              <li key={index} className="py-2">
                <p className="text-gray-700">
                  <span className="font-bold text-blue-700">{log.user_name}</span> - {log.action}{' '}
                  {log.description && (
